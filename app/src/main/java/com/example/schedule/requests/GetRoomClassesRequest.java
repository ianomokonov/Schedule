...
public class GetRoomClassesRequest extends AsyncTask<String, Void, ArrayList<Subject>> {
    Gson gson = new Gson();
    SubjectAdapter adapter;
    Calendar dateAndTime=Calendar.getInstance();
    Context context;
    ArrayList<Subject> subjects;
    public GetRoomClassesRequest(SubjectAdapter adapter, Context context, ArrayList<Subject> subjects){
    ...
    protected ArrayList<Subject> doInBackground(String... term){
        ...
        try{
            ...
            if(term.length > 2){
                subjectsDTO = gson.fromJson(ApiService.get("https://ruz.fa.ru/api/schedule/auditorium/"+term[0]+"?start="+term[1]+"&finish="+term[2]+"&lng=1"), SubjectDTO[].class);
            } else {
                subjectsDTO = gson.fromJson(ApiService.get("https://ruz.fa.ru/api/schedule/auditorium/"+term[0]+"?start="+term[1]+"&finish="+term[1]+"&lng=1"), SubjectDTO[].class);
            }
            int dayOfWeek = -1;
